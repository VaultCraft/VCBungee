...
public class SendVoteCommand extends Command {
    ...
    public void execute(CommandSender commandSender, String[] args) {
        if (commandSender.hasPermission("bungeecord.command.end") || (commandSender instanceof ProxiedPlayer && GroupUtil.hasPermission(NetworkUser.fromPlayer((ProxiedPlayer)commandSender).getGroups(), GroupUtil.Group.DEVELOPER))) {
            if (args.length == 0) {
